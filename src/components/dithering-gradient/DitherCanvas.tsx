...
export const DitherCanvas: React.FC<DitherCanvasProps> = ({
  ...
  const drawBlockGrid = useCallback((canvas: HTMLCanvasElement, logicalWidth: number, logicalHeight: number) => {
  ...
  const render = useCallback((logicalWidth: number, logicalHeight: number) => {
    ...
    imageData.data.set(processor.process(
      settings.mode,
      settings.levels,
  ...
  useEffect(() => {
    if (!containerRef.current || !canvasRef.current || !overlayRef.current) return;
